// PMO System - Time-Off Reject API Route
// PATCH /api/v1/time-off/[id]/reject - Reject time-off request (OWNER or ADMIN)
 ...
 * Reject a time-off request (OWNER or ADMIN).
 */
export const PATCH = withOrgScope(
  requireAdmin(async (req, { organizationId, session }) => {
    try {
      ...
      const result = await timeOffService.rejectTimeOff(timeOffId, organizationId, session.id);
      ...
      if ("error" in result) {
        return createErrorResponse(ERROR_CODES.FORBIDDEN, "Can only reject pending requests.", 403);
      }

      return Response.json(createSuccessResponse(result.data));
    } catch (error) {
      console.error("[PATCH /api/v1/time-off/[id]/reject]", error);
